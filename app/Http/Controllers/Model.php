...
function displayTexts($page){
  $sql = "SELECT text, title FROM ebe58_texts WHERE page = '$page'";
	$result = \DB::select($sql);

  return $result;
}

function updateText($text, $title){
  $sql = "UPDATE ebe58_texts SET text = '$text' WHERE title = '$title'";
  \DB::update($sql);
}
...
function activityView($id){
  $sql = "SELECT DISTINCT id, name, link, banner, text, text2, video, color, pdf, button, logo FROM ebe58_services WHERE id = '$id'";
  $result =\DB::select($sql)[0];

  return $result;
}

function addActivity($name, $link, $img_name, $text, $text2, $video, $color, $pdf_name, $logo_name, $btn_name){
  $sql = "INSERT INTO ebe58_services (name, link, banner, text, text2, video, color, pdf, button, logo) VALUES ('$name', '$link', '$img_name', '$text', '$text2', '$video', '$color', '$pdf_name', '$btn_name', '$logo_name')";
  \DB::insert($sql);
}

function activityUpdate($name, $link, $img_name, $text, $text2, $video, $color, $id, $pdf_name, $logo_name, $btn_name){
  $sql = "UPDATE ebe58_services SET name = '$name', link = '$link', banner = '$img_name', text = '$text', text2 = '$text2', video = '$video', color = '$color', pdf = '$pdf_name', logo = '$logo_name', button = '$btn_name' WHERE id = '$id'";
  \DB::update($sql);
}
